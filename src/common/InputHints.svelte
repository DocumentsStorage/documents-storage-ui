<script>
    ...
    const handleClickHint = (hint) => {
        clickedHint=true;
        dispatch('clickHint', {hint});
    };
...
{#each hints as hint}
    <li class="flex justify-between px-3 py-3 mt-1 rounded bg-gray-100 dark:bg-gray-700 cursor-pointer" on:click={()=>{handleClickHint(hint)}}>
    <small>
        {hint.length > 25
            ? hint.slice(0, 25) +
            "..."
